...
def generate_facilities_data(req_factory_cap, req_store_cap, req_office_cap):
    ...
    while current_cap < req_factory_cap:
        ...
        facilities.append(('factory', size, rent, None, 0))
        current_cap += size
    ...
    while current_cap < req_office_cap:
        ...
        facilities.append(('office', size, rent, None, 0))
        current_cap += size
    ...
    while current_cap < req_store_cap:
        ...
        facilities.append(('store', size, rent, access, 0))
        current_cap += size
...
def run_seed():
    ...
    num_npc_makers = max(1, int(demand / 200)) 
    num_npc_retailers = max(2, int(demand / 100))
    ...
    supplier_templates = [
        {"name": "Cheap Parts Co.", "score": 2.0, "cost": 0.8},
        {"name": "Standard Materials Inc.", "score": 3.0, "cost": 1.0},
        {"name": "Premium Tech Ltd.", "score": 4.5, "cost": 1.5}
    ]
    ...
    for part in gb.INDUSTRIES[gb.CURRENT_INDUSTRY]['parts']:
        for s in supplier_templates:
            ...
            """, (f"{s['name']} ({part['label']})", s['score'], s['cost'], part['key']))

    # 3. NPC生成 (従業員 + 失業者)
    npc_data_list = []
    
    # 企業ごとの必要人員計算と生成
    # メーカー (Player + NPC Makers)
    maker_ids = [player_id] + npc_maker_ids
    ...
    def add_employees(company_id, c_type, share):
        ...
        staff_req = {}
        if c_type == 'maker':
            # 生産能力: 需要の3.5倍確保 (前回要望の11.7係数相当: 11.7 * 0.3 = 3.51)
            # 1人あたり生産効率: gb.BASE_PRODUCTION_EFFICIENCY
            needed_prod = share / gb.BASE_PRODUCTION_EFFICIENCY
            staff_req[gb.DEPT_PRODUCTION] = max(1, int(needed_prod * 3.5))
            ...
            total_others = sum(staff_req.values())
            staff_req[gb.DEPT_HR] = max(1, int((total_others / gb.HR_CAPACITY_PER_PERSON) * 1.5))
        else:
            # 店舗能力: 需要の1.1倍確保 (1.56係数相当: 1.56 * 0.7 = 1.092)
            needed_store = share / gb.BASE_SALES_EFFICIENCY
            staff_req[gb.DEPT_STORE] = max(1, int(needed_store * 1.1))
            ...
            total_others = sum(staff_req.values())
            staff_req[gb.DEPT_HR] = max(1, int((total_others / gb.HR_CAPACITY_PER_PERSON) * 2.0))
        ...
        for dept, count in staff_req.items():
            ...
            # 施設要件加算
            if dept == gb.DEPT_PRODUCTION: c_fac_req['factory'] += count
            elif dept == gb.DEPT_STORE: c_fac_req['store'] += count
            else: c_fac_req['office'] += count
    ...
    for mid in maker_ids:
        design_id = db.execute_query("""
            ...
            VALUES (?, 'Start Model', 3.0, 3.0, 1.0, ?, ?, 'completed', 0, ?)
        """, (mid, initial_base_price, initial_base_price, json.dumps(model_t_parts)))
    ...
    for cid, req in company_facilities_req.items():
        ...
        for i in range(len(f_data)):
            # f_data is list of tuples: (type, size, rent, access, is_owned)
            # We need to insert company_id into the query or modify data
            # DB schema: type, size, rent, access_score, is_owned, company_id
            ...
            # Actually generate_facilities_data returns (type, size, rent, access, 0)
            # We want to set company_id and is_owned=0 (rented)
            ftype, size, rent, access, _ = f_data[i]
            # 賃貸契約済みとして登録
            db.execute_query("INSERT INTO facilities (type, size, rent, access_score, is_owned, company_id) VALUES (?, ?, ?, ?, 0, ?)", 
                             (ftype, size, rent, access, cid))
    ...
    if market_facilities:
        conn = db.get_connection()
        conn.executemany("INSERT INTO facilities (type, size, rent, access_score, is_owned) VALUES (?, ?, ?, ?, ?)", market_facilities)
        conn.commit()
        conn.close()
